...
| `apps/dashboard/src/components/ui/confirm-dialog.tsx` | Confirmation dialog (destructive/warning variants) |
| `apps/dashboard/src/components/ui/toaster.tsx` | Toast notification renderer |
| `apps/dashboard/src/hooks/use-toast.ts` | Toast state management hook |
...
| Date | Change | Files |
|---|---|---|
| 2026-03-09 | **Studio toolbar buttons wired up** — Moved zone management buttons (Add Zone, Grid, Delete, Duplicate) from inside ZoneBuilder canvas to the unified ZoneToolbar. Toolbar now controls ZoneBuilder creation mode via `isCreatingZone`/`onCreatingZoneChange` props. Added `useDeleteZone` hook (calls `DELETE /zones/:id`). Delete zone button shows ConfirmDialog before calling API. Zone ID resolution fixed: prefers numeric `id` over string `zone_id` for API calls. Toolbar shows Delete/Duplicate buttons contextually when a zone is selected. | useChannels.ts, ZoneBuilder.tsx, studio/page.tsx |
| 2026-03-09 | **Studio drag-and-drop UX overhaul** — Replaced classList-based drag feedback with React-state-driven approach. Added global `isDraggingContent` state to coordinate UI across sidebar and canvas. ContentLibraryCard: grip handle icon, custom drag ghost image (tilted, branded shadow), opacity/scale feedback while dragging, sidebar hint text changes during drag. ZoneBuilder: all zones pulse with dashed primary border during drag, hovered zone gets ring glow + scale + "Release to add" overlay with Plus icon, green checkmark success flash on drop (1.2s), proper `relatedTarget` check prevents flicker on dragLeave, elevated z-index for hovered zone. Empty zones show enhanced "Drop here" state. Resize handles upgraded to 3px with white border + shadow for visibility. | ZoneBuilder.tsx, studio/page.tsx |
| 2026-03-09 | **Redesign App Edit page** — Replaced tab-based layout with single-page collapsible sections (General Settings, App Configuration, Linked Content, Preview). Sticky compact header bar with app icon (gradient), status badge, unsaved indicator, and Save/Cancel actions always visible. Right sidebar with app info card (metadata rows with icons), content thumbnail preview, and "Danger Zone" delete section. Collapsible `Section` component with icon headers. Mobile sticky save bar. Removed unused Tabs import. | apps/[id]/edit/page.tsx |
| 2026-03-09 | **Replace native dialogs with UI components** — Replaced all `confirm()` and `alert()` calls with proper styled components. New `ConfirmDialog` component (destructive red + warning amber variants) wraps existing Dialog. New toast notification system (`Toaster` + `useToast` hook) built on `@radix-ui/react-toast` with Ocean Breeze styling. Toaster added to root layout. 4 confirm() → ConfirmDialog, 6 alert() → toast.error(). | confirm-dialog.tsx, toaster.tsx, use-toast.ts, layout.tsx, FolderTree.tsx, studio/page.tsx, apps/page.tsx, apps/[id]/edit/page.tsx, channels/[id]/page.tsx, content/page.tsx |
