...
const CACHE_PREFIX = 'pbi_report_'

export function PowerBIReportRenderer({ config }: PowerBIReportRendererProps) {
  const {
    report_id,
  ...
  const [cachedData, setCachedData] = useState<PowerBIReportConfig['_data'] | null>(null)
  ...
  useEffect(() => {
    if (_data?.embed_url) {
      ...
      try {
        const cached = localStorage.getItem(CACHE_PREFIX + report_id)
        if (cached) {
